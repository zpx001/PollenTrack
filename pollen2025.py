...
def process_image(image_path, subfolder_name, alpha, current_image_idx, total_images):
    print(f"正在处理第 {current_image_idx} 张图像，共 {total_images} 张: {image_path}")
    try:
...
def process_all_images(alpha=50):
    ...
    for subfolder_name in os.listdir(input_path):
        ...
        if os.path.isdir(subfolder_path):
            ...
            image_paths.extend(glob.glob(os.path.join(subfolder_path, '*.jpg')) +
    ...
    for idx, image_path in enumerate(image_paths, 1):
        process_image(image_path, 'general', alpha, idx, total_images)
    ...
    print(f"处理完毕。总共处理了 {total_images} 张图像。")
    print(f"总处理时间: {time.time() - start_time:.2f} 秒。")


# 调用函数处理所有图片，默认透明度为50
process_all_images(alpha=20)
